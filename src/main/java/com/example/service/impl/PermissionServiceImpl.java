...
public class PermissionServiceImpl extends ServiceImpl<SysPermissionMapper, SysPermission> implements PermissionService {
        ...
        switch (sysPermission.getType()) {
            case 1:
                if (parent != null) {
                    if (parent.getType() != 1) {
                        ...
                        throw new MyException(BaseResponseCode.OPERATION_MENU_PERMISSION_CATALOG_ERROR);
                    }
            ...
            case 2:
                if (parent == null || parent.getType() != 1) {
                    ...
                    throw new MyException(BaseResponseCode.OPERATION_MENU_PERMISSION_MENU_ERROR);
                }
                if (StringUtils.isEmpty(sysPermission.getUrl())) {
                    // 菜单权限的url不能为空
                    throw new MyException(BaseResponseCode.OPERATION_MENU_PERMISSION_URL_NOT_NULL);
                }
            ...
            case 3:
                if (parent == null || parent.getType() != 2) {
                    ...
                    throw new MyException(BaseResponseCode.OPERATION_MENU_PERMISSION_BTN_ERROR);
                }
                if (StringUtils.isEmpty(sysPermission.getPerms())) {
                    // 菜单权限的标识符不能为空
                    throw new MyException(BaseResponseCode.OPERATION_MENU_PERMISSION_URL_PERMS_NULL);
                }
                if (StringUtils.isEmpty(sysPermission.getUrl())) {
                    // 菜单权限的url不能为空
                    throw new MyException(BaseResponseCode.OPERATION_MENU_PERMISSION_URL_NOT_NULL);
    ...
    private void verifyForm(SysPermission sysPermission) {
        ...
        if (!StringUtils.isEmpty(sysPermission.getId())) {
            ...
            if (!list.isEmpty()) {
                throw new MyException(BaseResponseCode.OPERATION_MENU_PERMISSION_UPDATE);
            }
        }
    }
    ...
    public void deleted(String permissionId) {
        ...
        if (!childs.isEmpty()) {
            throw new MyException(BaseResponseCode.ROLE_PERMISSION_RELATION);
        }
